 ...
 * Returns a leading separator followed by one item per available other provider,
 * or an empty array when there are no available alternatives.
 */
export function buildResumeWithProviderItems(
  ...
  for (const p of others) {
    ...
    item.textContent = `Resume with ${p.displayName}`;
